    ...
    def listen_loop(self):
        ...
        while True:
            try:
                ...
                try:
                    text = self._recognizer.recognize_google(audio, language="ko-KR")
                    print(f"🗣 인식: {text}")
                    self._dispatch_text(text)
                except sr.UnknownValueError:
                    self.speak("명령을 인식하지 못했습니다.")
                except sr.RequestError:
                    self.speak("음성 인식 서버에 연결할 수 없습니다.")
            except Exception:
                time.sleep(1)
...
def main():
    ensure_gitignore()
    print("🚀 신세계 통합 프로젝트 가동 시작")
    initialize_ai_manager()

    args = parse_args()
    tool = RunAllTool(verbose=args.verbose)

    # CLI 동작(원하면 음성/웹 없이 이 모드만 써도 됨)
    if any([args.command, args.script, args.file, args.directory]):
        if args.command:
            r = tool.run_command(args.command)
            if not args.verbose: print(r['stdout'], end='')
            if r['stderr']: print(r['stderr'], file=sys.stderr, end='')
        if args.script:
            r = tool.run_script(args.script)
            if not args.verbose: print(r['stdout'], end='')
            if r['stderr']: print(r['stderr'], file=sys.stderr, end='')
        if args.file:
    ...
    # 웹 대시보드 + 음성
    if not args.headless and FLASK_OK:
        threading.Thread(target=run_dashboard, daemon=True).start()
        print("🌐 대시보드: http://localhost:5000")

    if not args.headless:
        VoiceCommandManager().listen_loop()
