    ...
    def listen_loop(self):
        ...
        while True:
             try:
                 ...
                 # 인식기 기본값 튜닝
                    self.recognizer.energy_threshold = 300            # 기본 소음 기준
                    self.recognizer.pause_threshold = 3.0             # 말 멈춘 뒤 3초 기다림
                    self.recognizer.dynamic_energy_threshold = True   # 자동 소음 보정 켜기
             
                 # 시작 시 반초 정도 주변 소음 샘플링 → 과민/둔감 방지
                    self.recognizer.adjust_for_ambient_noise(source, duration=0.5)
            
                 # 너무 오래 기다리거나 한 문장이 너무 길어지는 것 방지
                 audio = self.recognizer.listen(
                    source,
                    timeout=8,            # 8초 안에 말 시작
                 없으면 타임아웃 
                    phrase_time_limit=15   # 한 번 말할
                 때 15초까지만 듣기
             )
                text = 
             self.recognizer.recognize_google(audi
             o, language="ko-KR")
                print(f"🎤 인식: {text}")
                self.dispatch_text(text)
             try:
                   except sr.WaitTimeoutError:
                   pass # 조용하면 그냥 다음 루프로 넘어가며 대기
        
                   except sr.UnknownValueError:
                    self.speak("음성을 인식하지 못했습니다.")
                   except sr.RequestError:
                    self.speak("음성 서버에 연결할 수 없습니다.")
                   except Exception as e:
                    self.speak("예기치 못한 오류가 발생했습니다.")
        print(e)
