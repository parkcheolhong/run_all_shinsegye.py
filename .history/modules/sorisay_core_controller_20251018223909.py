...
class SorisayCore:
    ...
    def get_default_config(self):
        ...
        return {
            ...
            "tts": {
                "rate": 120,  # 150에서 120으로 속도 줄임
                "voice_index": 0,
                "volume": 0.8,  # 0.9에서 0.8로 볼륨 조정
                "voice_gender": "female",
                "language": "ko",
                "emotion": "calm",  # friendly에서 calm으로
                "pitch": 0,
                "voice_effects": {
                    "enable_emotion": False,  # 감정 표현 비활성화
                    "enable_speed_variation": False,  # 속도 변화 비활성화
                    "enable_pause_insertion": True  # 자연스러운 일시정지는 유지
                }
            }
        }
    ...
    def speak_with_emotion(self, text, emotion="neutral", speed_modifier=1.0):
        ...
        try:
            ...
            # 감정에 따른 속도 조절 - 매우 자연스럽게 (변화 최소화)
            if effects.get("enable_speed_variation", False):  # 기본적으로 비활성화
                emotion_rates = {
                    "excited": original_rate * 1.02,  # 거의 변화 없음
                    "happy": original_rate * 1.01,   # 거의 변화 없음
                    "neutral": original_rate,
                    "sad": original_rate * 0.98,     # 아주 살짝만 느리게
                    "error": original_rate * 0.99,   # 거의 변화 없음
                    "success": original_rate * 1.01, # 거의 변화 없음
                    "casual": original_rate * 0.99,  # 편안하게
                    "calm": original_rate * 0.97     # 차분하게 조금 느리게
                }
