...
class SorisayCore:
    def __init__(self, config_path="config/settings.json"):
        ...
        self.creative_mode = True  # 창조 모드 활성화
    ...
    def run(self):
        ...
        while self.running:
            ...
            time.sleep(0.5)
                        ...
                        # 🎨 창조 모드: 새로운 아이디어 제안
                        if self.creative_mode and random.random() < 0.3:  # 30% 확률로 창조적 제안
                            creative_idea = self.creative_engine.generate_creative_idea(cmd)
                            time.sleep(0.5)
                            self.speak(creative_idea)
                ...
                if plugin_name and command:
                    ...
                    if self.creative_mode:
                        suggestion = self.creative_engine.suggest_improvements()
                        web_info = self.creative_engine.web_search_and_learn(cmd)
                        
                        response = f"명령을 이해하지 못했지만, {web_info}. {suggestion}"
                        self.speak(response)
                        
                        # 학습에 추가
                        self.creative_engine.learn_from_interaction(cmd, response)
                    else:
