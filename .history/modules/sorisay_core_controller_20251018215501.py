        ...
        while self.running:
            ...
            if intent and confidence > 0.5:
                ...
                if plugin_name and command:
                    ...
                    try:
                        response = self.plugin_manager.execute_command(plugin_name, command, cmd)
                        # 성공적인 명령 실행시 성공 감정으로 응답
                        result_emotion = "success" if "완료" in response or "성공" in response else "neutral"
                        self.speak(response, result_emotion)
                    ...
                    self.speak("죄송합니다. 명령을 이해하지 못했습니다. '도움말'이라고 말씀하시면 사용 가능한 명령어를 알려드리겠습니다.", "sad")
                    broadcast_voice_command(cmd, "failed")
                    yield cmd
