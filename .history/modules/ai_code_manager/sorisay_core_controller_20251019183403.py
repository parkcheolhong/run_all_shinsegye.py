...
def create_autonomous_mall_response(user_request):
    ...
    # 쇼핑몰 자율 운영 시작
    mall_status = mall.start_autonomous_operation()
    
    response = f"🛒 지능형 자율 쇼핑몰 가동!\n\n"
    response += f"📊 AI 시장 분석: {mall_status['market_analysis']}\n"
    response += f"🎯 신제품 기획: {mall_status['product_planning']}\n"
    response += f"🚀 자동 런칭: {mall_status['launch_status']}\n"
    response += f"💰 실시간 판매: {mall_status['sales_status']}\n"
    response += f"🤖 AI 자동 구매: {mall_status['purchase_status']}\n\n"
